...
router.post('/', bookingLimiter, [
  ...
  if (date < today) return res.status(400).json({ error: 'Appointment date cannot be in the past.' });

  try {
    const clash = await Meeting.findOne({ date, time, status: { $ne: 'cancelled' } });
    if (clash) return res.status(409).json({ error: `The slot on ${date} at ${time} is already booked. Please choose a different time.` });
