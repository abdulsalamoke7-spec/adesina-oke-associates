...
router.post('/', [
  ...
  try {
    // ── Clash check ─────────────────────────────────────────────────────────
    // An active meeting exists if same date+time and NOT cancelled
    ...
    // Fire notifications (non-blocking — don't fail the response if they error)
...
// ── GET /api/meetings/slots?date=YYYY-MM-DD — available slots (public) ──
router.get('/slots', async (req, res) => {
  const { date } = req.query;
  if (!date) return res.status(400).json({ error: 'date query param required' });
